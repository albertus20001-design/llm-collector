...
Google 适合结合 function calling、结构化输出与模型能力一起看，但工具成本描述可能分散在功能文档和价格文档里。
...
Anthropic 更适合作为工具调用范式和消息结构参考，价格判断仍主要回到 pricing 文档本身。
...
OpenRouter 是理解 server tools / 平台工具抽象的重点对象。实际判断时，要把“平台层工具能力”与“底层 provider 成本”分开看。
...
DeepSeek 目前更适合作为函数调用和 JSON 输出能力补充样本，而不是 server tool 计费主样本。
